  ...
  return (
    <section className="card">
      ...
      <div className="actions small">
        ...
        <button onClick={() => setHopLevel(1)} className={hopLevel === 1 ? 'success' : ''}>1-hop</button>
        <button onClick={() => setHopLevel(2)} className={hopLevel === 2 ? 'success' : ''}>2-hop</button>
      </div>
      {search && <div className="actions small">{filteredCards.filter((card) => card.title.toLowerCase().includes(search.toLowerCase())).slice(0, 8).map((card) => <button key={card.id} onClick={() => setSelectedCardId(card.id)}>{card.title}</button>)}</div>}
      <div className="all-mode-layout">
        <div>
          <svg viewBox="0 0 400 400" className="graph-svg compact">
            {filteredEdges.map((edge) => {
              ...
              return <line key={edge.id} x1={from.x} y1={from.y} x2={to.x} y2={to.y} stroke={edge.scope === 'cross-unit' ? '#7c3aed' : '#9ca3af'} strokeDasharray={edge.scope === 'cross-unit' ? '4 3' : undefined} strokeWidth={1 + strength * 3} opacity={isNearSelected ? 0.95 : 0.2} />
            })}
            {points.map((point) => {
              ...
              return <g key={point.card.id} className="clickable" onClick={() => setSelectedCardId(point.card.id)}>
                <circle cx={point.x} cy={point.y} r={isSelected ? '22' : '16'} fill={isSelected ? '#111827' : isSelectedUnit ? '#334155' : '#6b7280'} opacity={inNeighborhood ? 1 : 0.4} />
        ...
        <article className="details-panel">
          {!selected ? <p>No card selected.</p> : <>
            ...
            <h4>Outgoing in-scope links</h4>
            {outgoingEdges.length === 0 ? <p>None</p> : <ul className="edge-list">{outgoingEdges.map((edge) => <li key={edge.id} className="edge-item"><div><strong>{edge.slot}</strong> → {findCard(graph.cards, edge.to)?.title ?? edge.to}</div><div>{edge.scope} · {edge.importance}</div><div>{edge.relationType ?? '—'}</div><div>{edge.cue}</div></li>)}</ul>}
            <h4>Incoming in-scope links</h4>
            {incomingEdges.length === 0 ? <p>None</p> : <ul className="edge-list">{incomingEdges.map((edge) => <li key={edge.id} className="edge-item"><div>{findCard(graph.cards, edge.from)?.title ?? edge.from} → {selected.title}</div><div>{edge.scope} · {edge.importance}</div><div>{edge.relationType ?? '—'}</div></li>)}</ul>}
          </>}
        </article>
      </div>
    </section>
